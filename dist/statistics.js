...
function updateSummaryStats(data) {
    ...
    if (totalVolunteers)
        totalVolunteers.textContent = data.summaryStats.totalVolunteers.toString();
    if (totalEvents)
        totalEvents.textContent = data.summaryStats.totalEvents.toString();
    if (avgHours)
        avgHours.textContent = data.summaryStats.avgHours.toFixed(1);
    if (satisfaction)
        satisfaction.textContent = `${data.summaryStats.satisfaction}%`;
}
function renderMonthlyBars(data) {
    const container = document.getElementById('monthlyBars');
    if (!container || !data.monthlyData?.labels)
        return;
...
function renderEventTypes(data) {
    ...
    if (!data.eventTypes?.labels || !legendContainer)
        return;
    const total = data.eventTypes.data.reduce((sum, val) => sum + val, 0);
    if (pieCenter)
        pieCenter.textContent = `${total} Events`;
    let legendHtml = '';
...
function renderYearlyGrowth(data) {
    const container = document.getElementById('yearlyBars');
    if (!container || !data.yearlyGrowth?.labels)
        return;
